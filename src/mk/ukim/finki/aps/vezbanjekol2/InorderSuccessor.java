...
class BinarySearchTree<E extends Comparable<E>> {
    ...
    public ArrayList<E> generateInOrderList(BNode<E> node, ArrayList<E> list) {
        if (node != null) {
            generateInOrderList(node.left, list);
            list.add(node.info);
            generateInOrderList(node.right, list);
        }
        return list;
    }

    public E getSuccessor(BNode<E> t, E x) {
        ArrayList<E> successorsList = generateInOrderList(root, new ArrayList<>());
        return successorsList.get(successorsList.indexOf(x) + 1);
